...
//        }
//
//        if(indexOfENBChose != -1){
//            if (exitBuffer[0].size() < EXB_SIZE) {
//                exitBuffer[0].push(entranceBuffer[indexOfENBChose].front()) ;
//                entranceBuffer[indexOfENBChose].pop();
//                //notify to Host increase count
//                // Time delay to send msg to Host is 1ms so that not necessarily to delay Credit_delay
//                send(new cMessage("NotifyHost"), "out", indexOfENBChose);
//
//            }
//        }
//
